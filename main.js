console.log("🚀 LockedIn: Gaze Keyboard + Live Cursor + Fixed Calibration (Keyboard-Bounded)");
...
// Calibration offset (computed automatically)
let offsetX = 0;
let offsetY = 0;
  ...
  if (gKey) {
    gKey.addEventListener(
      ...
      async () => {
        ...
        // Move cursor to the G key center
...
// === Gaze → Keyboard Mapping ===
function handleGazeSelection(gx, gy) {
  const previewRect = canvas.getBoundingClientRect();
  const keyboardRect = keyboard.getBoundingClientRect();

  // Normalize gaze coordinates (0–1)
  const normX = gx / canvas.width;
  const normY = gy / canvas.height;

  // Map gaze to keyboard area
  const pageX = keyboardRect.left + normX * keyboardRect.width + offsetX;
  const pageY = keyboardRect.top + normY * keyboardRect.height + offsetY;

  // Move red gaze cursor
  gazeCursor.style.transform = `translate(${pageX}px, ${pageY}px)`;

  // Detect which key is under gaze
